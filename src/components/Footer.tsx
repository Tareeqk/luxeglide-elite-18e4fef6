      ...
      <div className="container">
        <div className="flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="text-center md:text-start">
            ...
            <p className="text-background/50 text-sm max-w-xs">{t("footer.tagline")}</p>
          </div>

          <div className="flex gap-8">
            {links.map((link) => (
              <a key={link.href} href={link.href} className="text-background/50 hover:text-gold transition-colors text-sm">
                {link.label}
              </a>
            ))}
          </div>
